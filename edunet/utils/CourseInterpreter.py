...
# set 'if __name__ == '__main__' to run independently
if __name__ == '__main__':

    wnl = nl.WordNetLemmatizer()

    def CourseInterpreter(link, nouns_paragraph, nouns_lecture, transcript):
        ...
        class Lecture:
            ...
            # Editted by Rian Boutin for use on windows
            def nlp_pipeline(self):
                """Passes a document through html parsing, and nlp pipeline and returns a list of nouns"""
                # file_in = open(self.path) # original line
                file_in = open(self.path, encoding='utf-8') # editted line
        ...
        def get_course():
            ...
            link_splitted = args.link.split("/")
            if "openmedia.yale.edu" not in args.link:
            ...
            try:
                wget.download(args.link, out=downloaded_file)
                if os.path.exists(downloaded_file):
                    log("Downloaded the course at link: {}, at {}".format(args.link, downloaded_file), log_file)
                else:
                    log("Can NOT download the course at link: {}".format(args.link), log_file)
                    return False
            except:
                log("Can NOT download the course at link: {}".format(args.link), log_file)
                return False
        ...
        def get_transcript_file(course_name):
            ...
            if transcript:
                if transcript < 10:
                    transcript_file_pattern = "transcript0{}.html".format(args.transcript)
                else:
                    transcript_file_pattern = "transcript{}.html".format(args.transcript)
            else:
        ...
        def process_transcripts(transcripts, course_name):
            for transcript in transcripts:
                # transcript_name = transcript.split("/")[-1][:-5] # original line
                transcript_name = transcript.split("\\")[-1][:-5] # edited line
                log("Processing lecture {}".format(transcript_name), log_file)
                lz = TreeOfKnowledge(args.nb_keywords_per_paragraph, args.nb_keywords_per_lecture, transcript_name, transcript)
                lz.analysis()
                ...
                lecture_output_path = os.path.join(course_output_path, tree_name)
                lz.write_to_file(lecture_output_path)
                lz.print_tree()
    ...
    parser = argparse.ArgumentParser("Script to interpret courses")
    parser.add_argument("-l", "--link", required = True, type=str, help="Link to a new course")
    parser.add_argument("-np", "--nb_keywords_per_paragraph", required = True, type=int, help="nb_keywords_per_paragraph")
    parser.add_argument("-nl", "--nb_keywords_per_lecture", required = True, type=int, help="nb_keywords_per_lecture")
    parser.add_argument("-t", "--transcript", required = False, type=int, help="Transcript number")
    args = parser.parse_args()

    variables = vars(args) # create dictionary out of parsed arguments
    l = variables['link']
    npp = variables['nb_keywords_per_paragraph']
    npl = variables['nb_keywords_per_lecture']
    t = variables['transcript']

    CourseInterpreter(l, npp, npl, t)

else:
    print("File is trying to run indirectly.")
